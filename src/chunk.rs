...
impl Face {
    ...
    pub fn neighbors(&self) -> &'static [(Face, Edge, bool); 4] {
        use self::Face::*;
        match *self {
            PX => &[(NZ, Edge::NX, false), (PY, Edge::PX, false), (PZ, Edge::PX, false), (NY, Edge::PX, true)],
            NX => &[(PZ, Edge::NX, false), (PY, Edge::NX, true),  (NZ, Edge::PX, false), (NY, Edge::NX, false)],
            PY => &[(NX, Edge::NY, true),  (PZ, Edge::PY, true),  (PX, Edge::NY, false), (NZ, Edge::PY, false)],
            NY => &[(NX, Edge::PY, false), (NZ, Edge::NY, false), (PX, Edge::PY, true),  (PZ, Edge::NY, true)],
            PZ => &[(NX, Edge::NX, false), (NY, Edge::PY, true),  (PX, Edge::PX, false), (PY, Edge::NY, true)],
            NZ => &[(PX, Edge::NX, false), (NY, Edge::NY, false), (NX, Edge::PX, false), (PY, Edge::PY, false)],
        }
    }
}
...
impl ::std::ops::Neg for Edge {
    ...
    fn neg(self) -> Self {
        use self::Edge::*;
        match self { PX => NX, PY => NY, NX => PX, NY => PY }
    }
}
...
impl Chunk {
    ...
    pub fn root(face: Face) -> Self { Self { coords: (0, 0), depth: 0, face } }
    ...
    pub fn children(&self) -> [Self; 4] {
        ...
        let face = self.face;
        [Chunk { coords, depth, face },
         Chunk { coords: (coords.0, coords.1 + 1), depth, face },
         Chunk { coords: (coords.0 + 1, coords.1), depth, face },
         Chunk { coords: (coords.0 + 1, coords.1 + 1), depth, face }]
    }

    /// Chunks that share an edge with this chunk
    pub fn neighbors(&self) -> [Self; 4] {
        let Chunk { face, depth, coords } = *self;
        ...
        let neighbor_chunk = |face: Face, edge: Edge| {
            ...
            Chunk { face: neighboring_face, depth, coords }
        };
        [
            if coords.0 == 0 {
                neighbor_chunk(face, Edge::NX)
            } else {
                Chunk { face, depth, coords: (coords.0 - 1, coords.1) }
            },
            if coords.1 == 0 {
                neighbor_chunk(face, Edge::NY)
            } else {
                Chunk { face, depth, coords: (coords.0, coords.1 - 1) }
            },
            if coords.0 == max {
    ...
    pub fn edge_length(&self) -> f64 { 2.0 / 2u32.pow(self.depth as u32) as f64 }
    ...
    pub fn origin_on_face(&self) -> na::Unit<na::Vector3<f64>> {
        ...
        let vec = na::Vector3::new((self.coords.0 as f64 + 0.5) * size - 1.0,
    ...
    pub fn samples(self, resolution: u32) -> SampleIter {
        SampleIter { chunk: self, resolution, next: (0, 0) }
    }
}
...
impl Iterator for SampleIter {
    ...
    fn next(&mut self) -> Option<na::Unit<na::Vector3<f64>>> {
        if self.seq() == self.resolution as usize * self.resolution as usize { return None; }
        let edge_length = self.chunk.edge_length();
        let origin_on_face = na::Vector2::new(self.chunk.coords.0 as f64, self.chunk.coords.1 as f64) * edge_length - na::Vector2::new(1.0, 1.0);
        let step = self.chunk.edge_length() / (self.resolution - 1) as f64;
        let pos_on_face = origin_on_face + na::Vector2::new(self.next.0 as f64, self.next.1 as f64) * step;
...
mod test {
    ...
    fn face_neighbor_axes() {
        ...
        for face in Face::iter() {
            for (edge, &(neighbor, neighbor_edge, parallel)) in Edge::iter().zip(face.neighbors()) {
                let local = face.basis() * match edge {
                    Edge::PX | Edge::NX => na::Vector3::y(),
                    Edge::PY | Edge::NY => na::Vector3::x(),
                };
                let neighbor = neighbor.basis() * match neighbor_edge {
                    Edge::PX | Edge::NX => na::Vector3::y(),
                    Edge::PY | Edge::NY => na::Vector3::x(),
                };
    ...
    fn neighbors() {
        ...
        for face in Face::iter() {
            for (&(neighbor_face, _, _), &neighbor_chunk) in face.neighbors().iter().zip(&Chunk::root(face).neighbors()) {
                assert_eq!(Chunk::root(neighbor_face), neighbor_chunk);
            }
        }

        let chunk = Chunk::root(Face::PZ).children()[0];
        assert_eq!(chunk.neighbors()[0], Chunk { coords: (0, 1), depth: 1, face: NX });
        assert_eq!(chunk.neighbors()[1], Chunk { coords: (0, 1), depth: 1, face: NY });
        assert_eq!(chunk.neighbors()[2], Chunk { coords: (1, 0), depth: 1, face: PZ });
        assert_eq!(chunk.neighbors()[3], Chunk { coords: (0, 1), depth: 1, face: PZ });

        let chunk = Chunk { face: PX, depth: 1, coords: (1, 0) };
    ...
    fn sample_sanity() {
        ...
        for face in Face::iter() {
            ...
            for chunk in &Chunk::root(face).children() {
                ...
                assert_eq!(chunk.samples(2).filter(|&x| x == face.basis() * na::Vector3::z_axis()).count(), 1);
                // and another at a corner
                let corner = 1.0 / 3.0f64.sqrt();
                assert_eq!(chunk.samples(2).filter(|&x| x.x.abs() == corner && x.y.abs() == corner && x.z.abs() == corner).count(), 1);
            }
        }
    }
    ...
    #[test]
    fn origin_sanity() {
        assert_eq!(Chunk::root(Face::PZ).origin_on_face(), na::Vector3::z_axis());
    }
}
